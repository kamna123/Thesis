        ...
        if(lhs!=NULL)
         {
                   if(ref!=NULL)
                    {
                      SgExpression *exp;
                      if((stepast1->unparseToString()).compare("1")==1)
                       exp= buildVarRefExp("("+(ivarname)->unparseToString()+"*"+(stepast1)->unparseToString()+"+"+(lbast)->unparseToString()+")");
                      else
                       exp= buildVarRefExp("("+(ivarname)->unparseToString()+"+"+(lbast)->unparseToString()+")");
...
void visit1(SgNode* n)
{
     ...
     SgBasicBlock *block = isSgBasicBlock (a->get_loop_body());
            ...
            /*cout<<" loop stepast2= "<<(stepast1)->unparseToString()<<"\n";
              ...
              //SgExprStatement* exp=buildAssignStatement(buildVarRefExp((ivarname)->unparseToString()),buildIntVal(0));
               ...
               //prependStatement (lower_decl, scope);
                 ab=ubast->unparseToString();
             //    cout<<"ab = "<<ab<<endl;
               const char *ub_1=ab.c_str();
               int ub_2=atoi(ub_1);
               string ab1=(lbast)->unparseToString();
               const char *lb_1=ab1.c_str();
                int lb_2=atoi(lb_1);
              string ab2=(stepast1)->unparseToString();
                const char* step_1=ab2.c_str();
            
               
               
                int step_2=atoi(step_1);
                int final=(ub_2-lb_2)/step_2;
                
               //  cout<<"ub_2 " <<ub_1<<endl;
               // cout<<"lb_2 "<<lb_1<<endl;
               // cout<<"step "<<step_1<<endl;
