...
void visitorTraversal :: visit(SgNode* n)
{
     ...
     if(a!=NULL)
     {
      ...
      if(head_loop==NULL)
      {
         ...
         SgStatementPtrList & init3=a->get_init_stmt();
          if (isAssignmentStatement(init3, &ivarast, &lbast))
             {
                SgVarRefExp* var = isSgVarRefExp(ivarast);
                  if (var)
                   {
                     //cout<<"a"<<"\n";
                     ivarname = var->get_symbol()->get_declaration();
                     cout<<" loop lower bound new= "<<(lbast)->unparseToString()<<"\n"; 
                     cout<<" loop variable new= "<<(ivarname)->unparseToString()<<"\n";
                   }
             }   
